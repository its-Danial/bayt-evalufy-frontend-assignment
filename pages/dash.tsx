...
export default function DashboardHome() {
  ...
  const prioritySortedWidgets = [...widgetConfig].sort((a, b) => a.priority - b.priority); // Sort widgetConfig based on priority
  const [sortedWidgets, setSortedWidgets] = useState(prioritySortedWidgets);
  const [widgetIsLoading, setWidgetIsLoading] = useState(true); // Introduced a loading state
        ...
        <div className="grid gap-5 mt-6" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))" }}>
          {sortedWidgets.map((widget, index) => (
            <Widget
              ...
              type={widget.type}
              title={widget.title}
              data={widgetData && widgetData.find((item) => item.id === widget.id)!}
              gridPosition={{
